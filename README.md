...
> `auth login` 使用 Chat 服务基址，默认请求 `POST https://coreinsight.rnd.huawei.com/chat/api/auth/login`，请求体为 `{userName, passwd}`，并从响应 `Set-Cookie` 建立本地会话。可用 `COREINSIGHT_CHAT_SERVER` 或 `--chat-server` 覆盖基址。
